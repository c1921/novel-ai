...
`novel-cli` 从三处来源合并配置。本文档涵盖每个配置项、优先级以及如何配置。
...
| 层级 | 来源 | 说明 |
|---|---|---|
| 1（最高） | 环境变量 / 项目 `.env` | 运行时覆盖；API key 仅在此层 |
| 2（中等） | 项目 `novel.yaml` | 每个项目独立设置 |
| 3（最低） | 用户级 `config.yaml` | 跨所有项目的全局默认值 |
...
环境变量始终具有最高优先级。可在系统全局设置，也可写入项目 `.env` 文件（由 `python-dotenv` 加载）。
...
| Model | `NOVEL_MODEL` > 项目 `model.name` > 用户 `model.name` > `gpt-4.1-mini` |
| Temperature | `NOVEL_TEMPERATURE` > 项目 `model.temperature` > 用户 `model.temperature` > `0.7` |
