...
本 CLI 应用用于封装 OpenAI-compatible API 调用能力，为多个小说项目提供统一的续写、润色、改写、摘要、上下文预览等命令行接口。
...
```text
全局 novel CLI：稳定执行写作任务、调用 OpenAI-compatible API、保存输出
...
6. 调用 OpenAI-compatible API 获取生成结果。
...
Codex 不应该每次临时拼接复杂的 Python 命令，也不应该直接处理 OpenAI-compatible API 细节。更推荐的分工如下：

```text
Codex：理解用户意图、选择文件、调用命令、检查结果
全局 CLI：稳定执行写作任务、调用 OpenAI-compatible API、保存输出
...
| `api_client.py` | 调用 OpenAI-compatible API |
...
name = "novel-cli"
version = "0.1.0"
description = "Global CLI for novel writing with OpenAI-compatible API"
...
OpenAI-compatible API key 不应写入代码、prompt、skill、AGENTS.md 或 `novel.yaml`。
...
CLI 启动时应读取 `.env` 和系统环境变量。如果缺少 `NOVEL_API_KEY`，应给出清晰错误：

```text
Missing NOVEL_API_KEY. Please set it in your environment or project .env file.
```
...
--model <name>             指定 OpenAI-compatible API 模型
...
用于预览将发送给 OpenAI-compatible API 的上下文。
...
此命令不调用 OpenAI-compatible API，只输出：
...
## 11. OpenAI-compatible API 调用设计

`api_client.py` 应只负责 API 调用，不负责项目业务逻辑。
...
1. 从环境变量读取 `NOVEL_API_KEY`。
2. 从环境变量、用户级配置、项目配置或参数读取 `NOVEL_BASE_URL`。
3. 使用 OpenAI SDK 或其他 OpenAI-compatible API 实现调用目标服务。
...
{
  "status": "error",
  "error": "Missing NOVEL_API_KEY",
  "hint": "Set NOVEL_API_KEY in your environment or project .env file."
}
```

## 14. `--dry-run` 设计

`--dry-run` 用于调试 prompt，不调用 OpenAI-compatible API。
...
```text
.agents/skills/novel-api/SKILL.md
```
...
---
name: novel-api
description: use this skill when working in a fiction project and the user asks to continue, polish, rewrite, summarize, outline, or revise novel chapters using the globally installed novel CLI and OpenAI-compatible API while preserving story continuity, character voice, timeline, and worldbuilding.
---
...
3. Codex 是流程编排层，不直接处理 OpenAI-compatible API 细节。
...
全局 CLI 定位当前小说项目
        ↓
CLI 读取 novel.yaml、上下文和 prompt，并调用 OpenAI-compatible API
        ↓
